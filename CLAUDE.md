...
- **怪我タブ追加**：`docs/injury-tab-plan.md` を参照。フェーズ1（閲覧）→フェーズ2（編集）の段階実装。
...
| 17 (Q) | **exInstanceId**（フロントで生成、`'exinst_' + Date.now() + '_' + index`） |
| 18 (R) | **duration**（セット所要時間・秒、タイマー計測値） |
...
- 記録シートのcol 17（exInstanceId）は同セッション内の同種目重複・複数出現を区別するために使用（`updateExerciseRecords` で行特定に使用）

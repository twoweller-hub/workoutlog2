...
### 別セットの「開始」を押したら前のカウントをリセット

- 別セットの「開始」押下時に既存の `.started` ボタンを探してリセット（`startedAt = null`・クラス除去・テキストを「開始」に戻す）
- これにより前のカウント中の点滅・緑枠も消える
- キャッシュバスター: `app.js?v=44`・SW `workoutlog2-v76`
